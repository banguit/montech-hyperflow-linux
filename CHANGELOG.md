...
- **Start display at boot** and **Show tray icon at login** in that menu.
  Deliberately two settings: the service starts at boot and drives the head
  with nobody logged in, the tray starts at login and only reports on it.
  Without these, a service started from the menu silently did not survive a
  reboot — `systemctl start` and `systemctl enable` are different things and
...
- **Every product reference now says HyperFlow _Digital_.** Montech sells the
  HyperFlow and the HyperFlow Digital and only the Digital has a screen. The
  deb and snap descriptions, the AppImage desktop entry, the tray title and
  the About dialog all said plain "HyperFlow", which would tell an owner of
  the screenless model that this was for them.
